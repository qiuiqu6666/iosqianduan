...
- (UIPageControl *)pageControl {
    if (_pageControl == nil) {
        ...
        _pageControl.currentPageIndicatorTintColor = HexColor(0x7a7c7f);
        _pageControl.pageIndicatorTintColor = HexColor(0xdee0e4);
        _pageControl.frame = CGRectMake(0, 0, kHcdGuideViewBounds.size.width, 44.0f);
        _pageControl.center = CGPointMake(kHcdGuideViewBounds.size.width / 2, kHcdGuideViewBounds.size.height - 35);
    }
    return _pageControl;
}
...
//  cell.imageView.center = CGPointMake(kHcdGuideViewBounds.size.width / 2, kHcdGuideViewBounds.size.height / 2);
    ...
    if (indexPath.row == self.images.count - 1) {
        [cell.buttonEnd setHidden:NO];
        [cell.buttonEnd addTarget:self action:@selector(nextButtonHandler:) forControlEvents:UIControlEventTouchUpInside];
        [cell.buttonEnd setBackgroundColor:self.buttonBgColor];
        [cell.buttonEnd setTitle:self.buttonTitle forState:UIControlStateNormal];
        [cell.buttonEnd setTitleColor:self.titleColor forState:UIControlStateNormal];
        cell.buttonEnd.layer.borderColor = [self.buttonBorderColor CGColor];
    } else {
        [cell.buttonEnd setHidden:YES];
    }

    [cell.buttonSkip addTarget:self action:@selector(nextButtonHandler:) forControlEvents:UIControlEventTouchUpInside];
