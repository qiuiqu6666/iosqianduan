...
final class TabBarSegmentedControl: UISegmentedControl {
    ...
    /// Tag used by host app for badge container (skip hiding its label in hideLabelsRecursively).
    private static let badgeContainerTag = 8_888
    ...
    private func hideLabelsRecursively(in view: UIView) {
        if let label = view as? UILabel,
           ...
           label.superview?.tag != Self.badgeContainerTag {
            label.isHidden = true
        }
        for subview in view.subviews {
            hideLabelsRecursively(in: subview)
    ...
    /// 切换 Tab 时的系统「滑块」液态透镜；设为透明，仅保留其几何供 `currentIndicatorRect()` 做选中高亮遮罩。
    private func hideSelectionIndicatorVisual() {
        let indicator = findIndicatorView()
        cachedIndicatorView = indicator
