...
final class GlassTabBarView: UIView {
    ...
    private var fabLongPressAction: (() -> Void)?
    ...
    init(
        ...
        self.fabLongPressAction = action.longPressAction
    ...
    private func setupViews(action: FabBarAction) {
        ...
        if fabLongPressAction != nil {
            let longPress = UILongPressGestureRecognizer(target: self, action: #selector(handleFabLongPress(_:)))
            longPress.minimumPressDuration = 0.45
            fabButton.addGestureRecognizer(longPress)
        }
        ...
        NSLayoutConstraint.activate([
            ...
            // FAB glass view：圆角与宽高比见下方；避免与「宽度被压到 0」时 width==height 硬冲突（控制台 NSLayoutConstraint width==0）
        ...
        // FAB：禁止宽度被压成 0（与 width==height 叠加曾触发控制台约束冲突）；高度由 top/bottom 决定，不设 minHeight 以免矮于 bar 内容区时与父视图顶底冲突
        fabGlassView.widthAnchor.constraint(greaterThanOrEqualToConstant: 44).isActive = true
        let fabAspect = fabGlassView.widthAnchor.constraint(equalTo: fabGlassView.heightAnchor)
        fabAspect.priority = UILayoutPriority(999)
        fabAspect.isActive = true
        fabGlassView.setContentCompressionResistancePriority(.required, for: .horizontal)
        fabGlassView.setContentCompressionResistancePriority(.required, for: .vertical)
    }
    ...
    @objc private func handleFabLongPress(_ gesture: UILongPressGestureRecognizer) {
