...
struct FabBarRepresentable<Value: Hashable>: UIViewRepresentable {
    ...
    /// 与 `FabBar.selectedTabAccentColor` 一致；有值则优先于继承的 `tintColor`。
    var selectedTabAccentColor: UIColor?
    ...
    func makeUIView(context: Context) -> GlassTabBarView {
        ...
        Self.applyActiveTint(control: control, uiView: container, fixedAccent: selectedTabAccentColor)
    ...
    func updateUIView(_ uiView: GlassTabBarView, context: Context) {
        ...
        Self.applyActiveTint(control: control, uiView: uiView, fixedAccent: selectedTabAccentColor)
    }

    /// 选中段 `activeTintColor`：固定色优先（首帧即可正确），否则从 `tintColor` 继承。
    private static func applyActiveTint(control: TabBarSegmentedControl, uiView: UIView, fixedAccent: UIColor?) {
        if let fixed = fixedAccent {
            control.activeTintColor = UIColor(cgColor: fixed.cgColor)
            return
        }
    ...
    private func configureSegmentContent(on control: TabBarSegmentedControl) {
        ...
        // Two views per segment: base (inactive) uses image; accent (active) uses selectedImage ?? image.
        let baseViews = tabs.map { makeContentView(for: $0, useSelectedImage: false) }
        let accentViews = tabs.map { makeContentView(for: $0, useSelectedImage: true) }
    ...
    private func makeContentView(for tab: FabBarTab<Value>, useSelectedImage: Bool) -> TabItemContentView {
        if useSelectedImage, let sel = tab.selectedImage {
            return TabItemContentView(title: tab.title, imageName: sel, imageBundle: tab.selectedImageBundle)
        }
        if let imageName = tab.image {
            return TabItemContentView(title: tab.title, imageName: imageName, imageBundle: tab.imageBundle)
        }
        return TabItemContentView(title: tab.title, symbolName: tab.systemImage ?? "")
    }
