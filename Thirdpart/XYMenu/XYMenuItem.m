...
static const CGFloat XYMenuItemLabelWidth = 70; // item内的label宽度, add by Jack Jiang
...
- (void)setUpViewsWithRect:(CGRect)rect
{
    ...
    self.titleLab.frame = CGRectMake(iconMaxY + (iconHeight * 3 / 4), iconHeight
                                     , XYMenuItemLabelWidth//iconHeight * 3
                                     , iconHeight);
}
...
- (UILabel *)titleLab
{
    if (!_titleLab) {
        ...
        _titleLab.textColor = [UIColor colorWithRed:3.0f/255.0f green:6.0f/255.0f blue:21.0f/255.0f alpha:1.0f];//[UIColor whiteColor];
