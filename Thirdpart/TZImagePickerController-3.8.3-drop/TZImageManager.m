...
- (void)getCameraRollAlbumWithFetchAssets:(BOOL)needFetchAssets completion:(void (^)(TZAlbumModel *model))completion {
    ...
    // allowPickingVideo=NO 时只拉取图片，避免进入「视频」相册后过滤掉全部视频导致显示「无照片或视频」；GIF 多为 Image 类型，由 tz_getAssetFilename 识别
...
- (NSString *)tz_getAssetFilename:(PHAsset *)phAsset {
    ...
    if (@available(iOS 9.0, *)) {
        ...
        for (PHAssetResource *res in resources) {
            NSString *orig = res.originalFilename;
...
- (TZAssetModelMediaType)getAssetType:(PHAsset *)asset {
    ...
    PHAsset *phAsset = (PHAsset *)asset;
    NSString *name = [self tz_getAssetFilename:phAsset];
    NSString *nameLower = [name isKindOfClass:[NSString class]] ? [name lowercaseString] : nil;
    BOOL isGifByName = (nameLower.length >= 4 && [nameLower hasSuffix:@"gif"]);
    if (phAsset.mediaType == PHAssetMediaTypeVideo) {
        // 部分系统/相册会把 GIF 存为 Video，按文件名识别为动图，避免被 allowPickingVideo=NO 过滤掉
        type = isGifByName ? TZAssetModelMediaTypePhotoGif : TZAssetModelMediaTypeVideo;
...
- (PHImageRequestID)getOriginalPhotoDataWithAsset:(PHAsset *)asset progressHandler:(void (^)(double progress, NSError *error, BOOL *stop, NSDictionary *info))progressHandler completion:(void (^)(NSData *data,NSDictionary *info,BOOL isDegraded))completion {
    PHImageRequestOptions *option = [[PHImageRequestOptions alloc] init];
    option.networkAccessAllowed = YES;
    NSString *fname = [self tz_getAssetFilename:asset];
    if (fname.length >= 4 && [[fname lowercaseString] hasSuffix:@"gif"]) {
