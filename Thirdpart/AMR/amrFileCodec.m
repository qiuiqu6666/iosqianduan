...
NSString* EncodePCMToAMR(char* data, int maxLen,int nChannels, int nBitsPerSample, NSString *destAMRFileDir)
{
    char* oldBuf = data;
    /* input speech vector */
	short speech[160];
...
//    NSString *documentPath       = [paths objectAtIndex:0];
    // 尝试创建目录（如果不存的话）
    [FileTool tryCreateDirs:destAMRFileDir];

    NSString *wavFile        = [destAMRFileDir stringByAppendingPathComponent:[NSString stringWithFormat:@"%d_%@.amr",frames*20, [md5 md5HashOfData:out]]];
...
NSString* EncodeWAVEToAMR(NSData* data, int nChannels, int nBitsPerSample, NSString *destAMRFileDir)
{
//    NSArray *paths               = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
//    NSString *documentPath       = [paths objectAtIndex:0];
//    NSLog(@"documentPath=%@", documentPath);
    ...
    return EncodePCMToAMR(buf,maxLen- nPos,nChannels,nBitsPerSample, destAMRFileDir);
}
